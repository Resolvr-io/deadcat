...
- **Deadcat Rust workspace** reorganized to accommodate the new crates. Concretely, the workspace root grows a `crates/deadcat-core/` and `crates/deadcat-codegen/` directory. Physical layout under `deadcat-core`:
  ...
  crates/deadcat-core/
    ...
    src/
      ...
  ```
  `deadcat-codegen` writes to the `contracts/multi_outcome/` path inside `deadcat-core`'s directory; `deadcat-core` reads those files via `include_bytes!`.
...
4. **Pool lifecycle at market resolution** — edge cases around a pool whose parent market resolved or expired. v1 treatment (e.g., pool admin-close path remains available regardless of market state; trading halts when market is no longer Trading). Document the chosen behavior.
...
- **`verify_integration(xprv, chain_source)` sanity helper** — v1-adjacent ergonomic improvement for wallet integrators; non-blocking.
