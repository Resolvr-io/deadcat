...
1. **Trade routing** — `quote_trade` on the engine + `build_trade_pset` per [trade-routing-algorithm.md](trade-routing-algorithm.md). Composes pool swaps with maker order fills in a single atomic transaction. `build_trade_pset` returns `PreBlindedPset` so plain and market-assisted routes share one wallet signing flow.
