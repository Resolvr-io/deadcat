...
import { renderWalletRecovery, renderWalletSetup } from "./wallet/setup.ts";
...
export function renderWallet(): string {
  ...
  if (state.walletRecovering || state.walletRecoverySummary) {
    return renderWalletRecovery({ networkBadge });
  }
  ...
  return `
    <div data-action="close-wallet-backdrop" class="fixed inset-0 z-40 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div class="relative mx-4 flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-slate-700 bg-slate-950 shadow-2xl">
        ...
        <div data-wallet-scroll class="flex-1 overflow-y-auto">
          ${content}
        </div>
      </div>
    </div>
