...
export function renderWalletUtxoSection(params: {
  ...
  const lbtcRows = lbtcUtxos
    .map((u) =>
      utxoRow(
        u,
        '<span class="rounded bg-slate-700 px-1.5 py-0.5 text-[10px] font-medium text-slate-300 shrink-0">' + btcLabel() + '</span>',
      ),
    )
    .join("");
  ...
  const expandedContent = state.walletUtxosExpanded
    ? '<div class="mt-3">' +
      (lbtcUtxos.length > 0
        ? '<div class="mb-1 text-[10px] font-medium uppercase tracking-wider text-slate-500">' + btcLabel() + '</div>' +
          lbtcRows
        : "") +
      (tokenUtxos.length > 0
