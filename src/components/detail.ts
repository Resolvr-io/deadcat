...
export function renderActionTicket(market: Market): string {
  ...
  const currentDirection = state.tradeIntent === "open" ? "buy" : "sell";
  ...
  const snapshot = state.tradeQuoteSnapshot;
  const quote =
    snapshot &&
    snapshot.marketId === market.id &&
    snapshot.side === state.selectedSide &&
    snapshot.direction === currentDirection &&
    snapshot.exactInput === currentExactInput
      ? snapshot.quote
      : null;
  const tradeBusy = state.tradeQuoteLoading || state.tradeExecuteLoading;
  ...
  const routeLegsHtml = quote
    ? quote.legs
        .map((leg, idx) => {
          ...
          return `<div class="flex items-center justify-between rounded border border-slate-800 bg-slate-900/50 px-2 py-1"><span class="text-slate-300">${idx + 1}. ${sourceLabel}</span><span class="text-slate-400">${formatTradeAmount(leg.input_amount, inputUnit)} -> ${formatTradeAmount(leg.output_amount, outputUnit)}</span></div>`;
        })
        .join("")
    : "";
  return `
    <aside class="rounded-[21px] border border-slate-800 bg-slate-900/80 p-[21px]">
      ...
      <div class="mb-3 flex items-center justify-between gap-2">
        ...
        <div class="grid grid-cols-2 gap-2">
          <button data-size-mode="sats" ${state.tradeIntent === "close" ? "disabled" : ""} class="rounded border px-2 py-1 text-xs ${state.sizeMode === "sats" ? "border-slate-500 bg-slate-700 text-slate-100" : "border-slate-700 text-slate-300"} ${state.tradeIntent === "close" ? "cursor-not-allowed opacity-50" : ""}">sats</button>
          <button data-size-mode="contracts" ${state.tradeIntent === "open" ? "disabled" : ""} class="rounded border px-2 py-1 text-xs ${state.sizeMode === "contracts" ? "border-slate-500 bg-slate-700 text-slate-100" : "border-slate-700 text-slate-300"} ${state.tradeIntent === "open" ? "cursor-not-allowed opacity-50" : ""}">contracts</button>
        </div>
      </div>
      ${
        state.tradeIntent === "open"
      ...
      ${
        state.tradeQuoteLoading
          ? `<p class="mt-3 text-xs text-slate-400">Requesting live LMSR quote...</p>`
          : ""
      }
      ${
        quote
          ? `<div class="mt-3 rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-xs">
        <p class="mb-2 font-semibold text-slate-200">Live route quote</p>
        <div class="mb-2 flex items-center justify-between text-slate-400"><span>Total input</span><span>${formatTradeAmount(quote.total_input, inputUnit)}</span></div>
        <div class="mb-2 flex items-center justify-between text-slate-400"><span>Total output</span><span>${formatTradeAmount(quote.total_output, outputUnit)}</span></div>
        <div class="mb-2 flex items-center justify-between text-slate-500"><span>Effective price</span><span>${quote.effective_price.toFixed(6)}</span></div>
        <div class="space-y-1">${routeLegsHtml}</div>
      </div>`
          : `<p class="mt-3 text-xs text-slate-500">No live route quote yet for current trade inputs.</p>`
      }
      ${
        state.tradeError
