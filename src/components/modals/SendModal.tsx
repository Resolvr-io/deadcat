...
export function SendModal() {
  ...
  const sendLiquidAddress = useStore((s) => s.sendLiquidAddress);
  const sendLiquidAmount = useStore((s) => s.sendLiquidAmount);
  const sendBtcAmount = useStore((s) => s.sendBtcAmount);
  const sendCreating = useStore((s) => s.sendCreating);
  const sendError = useStore((s) => s.sendError);
  const sentLightningSwap = useStore((s) => s.sentLightningSwap);
  const sentLiquidResult = useStore((s) => s.sentLiquidResult);
  const sentBitcoinSwap = useStore((s) => s.sentBitcoinSwap);
  ...
  const createBitcoinSend = useCreateBitcoinSend();

  const creating =
    sendCreating ||
    payLightning.isPending ||
    sendLiquid.isPending ||
    createBitcoinSend.isPending;

  const handlePayLightning = useCallback(() => {
    const invoice = sendInvoice.trim();
    if (!invoice) {
      useStore.setState({ sendError: "Paste a BOLT11 invoice." });
      return;
    }
  ...
  const handleSendLiquid = useCallback(() => {
    ...
    if (!address || amountSat <= 0) {
      useStore.setState({ sendError: "Enter address and amount." });
      return;
    }
    useStore.setState({ sendCreating: true, sendError: "" });
    sendLiquid.mutate(
  ...
  const handleCreateBitcoinSend = useCallback(async () => {
    ...
    if (amt <= 0) {
      useStore.setState({ sendError: "Enter a valid amount." });
      return;
    }
    let pairInfo = sendBtcPairInfo;
    if (!pairInfo) {
      try {
        const pairs = await invoke<BoltzChainSwapPairsInfo>(
          "get_chain_swap_pairs",
        );
        pairInfo = pairs.liquidToBitcoin;
        useStore.setState({ sendBtcPairInfo: pairInfo });
      } catch (e) {
    ...
    useStore.setState({ sendCreating: true, sendError: "" });
    createBitcoinSend.mutate(
      { amountSat: amt, pairInfo },
      {
        onSuccess: async (swap) => {
          const addr = swap.claimLockupAddress;
          const qr = await generateQr(swap.bip21 || addr);
          useStore.setState({
            sentBitcoinSwap: swap,
            sendCreating: false,
            modalQr: qr,
          });
        },
  ...
  }, [sendBtcAmount, sendBtcPairInfo, createBitcoinSend]);

  let content: React.ReactNode = null;

  if (walletModalTab === "lightning") {
    if (sentLightningSwap) {
      const s = sentLightningSwap;
      content = (
        <div className="space-y-3">
          ...
          <Copyable value={s.lockupAddress} label="Lockup Address" />
          <Copyable value={s.bip21} label="BIP21 URI" />
        </div>
      );
    } else {
      content = (
        <div className="space-y-3">
          <p className="text-sm text-slate-400">
            Paste a BOLT11 Lightning invoice to pay via Boltz submarine swap.
          </p>
          <input
            id="send-invoice"
            value={sendInvoice}
    ...
    if (sentLiquidResult) {
      ...
      content = (
        <div className="space-y-3">
          <p className="text-sm text-slate-400">
            Send {btcLabel(showLbtcLabel)} directly to a Liquid address.
          </p>
          <input
            id="send-liquid-address"
            value={sendLiquidAddress}
            onChange={(e) =>
              useStore.setState({ sendLiquidAddress: e.target.value })
            }
    ...
    if (sentBitcoinSwap) {
      ...
      content = (
        <div className="space-y-3">
          <p className="text-sm text-slate-400">
            Create an L-BTC to BTC chain swap via Boltz.
          </p>
          {sendBtcPairInfo && (
            <div className="rounded-lg border border-slate-700 bg-slate-900 p-3 text-xs text-slate-400 space-y-1">
              <div>
                Min: {sendBtcPairInfo.minAmountSat.toLocaleString()} sats
              </div>
              <div>
                Max: {sendBtcPairInfo.maxAmountSat.toLocaleString()} sats
              </div>
              <div>
                Fee: {sendBtcPairInfo.feePercentage}% +{" "}
                {sendBtcPairInfo.fixedMinerFeeTotalSat.toLocaleString()} sats
                fixed
              </div>
            </div>
          )}
          <div className="flex gap-2">
            <input
              ...
              value={sendBtcAmount}
              onChange={(e) =>
                useStore.setState({ sendBtcAmount: e.target.value })
              }
              placeholder="Amount (sats)"
