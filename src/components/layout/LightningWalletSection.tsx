...
/**
 * Settings → Lightning wallet accordion body. Sits directly below
 * the Liquid wallet section. Single Connect button when disconnected,
 * alias + Disconnect when connected. A small "What's this?" link
 * opens an explainer dialog covering the Liquid-vs-Lightning split.
 */
export function LightningWalletSection() {
  const { connected, alias, disconnect } = useBcConnection();
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs leading-relaxed text-slate-400">
        Attach an external Lightning wallet via Bitcoin Connect so Deadcat can
        zap comments and (soon) bridge value to your Liquid balance.{" "}
        <button
          type="button"
      ...
      {connected ? (
        <div className="flex items-center gap-2 rounded-lg border border-slate-800 bg-slate-900/60 px-3 py-2.5">
          <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-emerald-400/15">
            <svg
              aria-hidden="true"
              className="h-[18px] w-[14px] text-emerald-400"
              viewBox="0 0 14 18"
              fill="currentColor"
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-medium text-slate-100">
              {alias || "Lightning wallet"}
            </p>
            <p className="text-xs text-slate-500">Connected via NWC</p>
          </div>
          <button
            type="button"
            onClick={() => void handleDisconnect()}
            disabled={disconnecting}
            className="shrink-0 rounded-md border border-slate-700 px-3 py-1.5 text-xs text-slate-300 transition hover:bg-slate-800 disabled:opacity-50"
          >
        ...
        <button
          type="button"
          onClick={() => launchModal()}
          className="flex w-full items-center justify-center gap-2 rounded-lg bg-emerald-400 px-4 py-2.5 text-sm font-semibold text-slate-950 transition hover:bg-emerald-300"
        >
          <svg
            aria-hidden="true"
            className="h-4 w-4"
