    ...
    return `
      <button data-open-market="${escapeAttr(market.id)}" class="rounded-2xl border border-slate-800 bg-slate-950/55 p-4 text-left transition hover:border-slate-600">
        ...
        <div class="flex items-center justify-between text-sm">
          <span class="text-emerald-300">${market.yesPrice != null ? `Yes ${Math.round(market.yesPrice * 100)}%` : "Buy Yes"}</span>
          <span class="text-rose-300">${no != null ? `No ${Math.round(no * 100)}%` : "Buy No"}</span>
        </div>
      </button>
    `;
  };
